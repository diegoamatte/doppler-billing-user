...
namespace Doppler.BillingUser.Mappers.BillingCredit
{
    public class BillingCreditForTransferMapper : IBillingCreditMapper
    {
        ...
        public async Task<BillingCreditAgreement> MapToBillingCreditAgreement(AgreementInformation agreementInformation, UserBillingInformation user, UserTypePlanInformation newUserTypePlan, Promotion promotion)
        {
            ...
            {
                Date = now,
                PaymentDate = !BillingHelper.IsUpgradePending(user, promotion) ? now : null,
                ActivationDate = !BillingHelper.IsUpgradePending(user, promotion) ? now : null,
                Approved = !BillingHelper.IsUpgradePending(user, promotion),
                Payed = !BillingHelper.IsUpgradePending(user, promotion),
