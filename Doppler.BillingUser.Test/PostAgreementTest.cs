...
namespace Doppler.BillingUser.Test
{
    public class PostAgreementTest : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        public async void POST_agreement_should_increment_times_to_use_of_promocode_when_agreement_is_accomplished()
        {
            ...
            // Assert
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            promotionRepositoryMock.Verify(x => x.UpdateTimeToUse(It.IsAny<Promotion>(), "+"), Times.Once());
        }

        [Fact]
        public async void POST_agreement_should_decrement_times_to_use_of_promocode_when_first_data_return_an_exception()
        {
            // Arrange
            const string accountName = "test1@test.com";
            var agreement = new
        ...
        [Fact]
        public async void POST_agreement_should_decrement_times_to_use_of_promocode_when_encrypted_credit_card_is_null()
        {
            // Arrange
            const string accountName = "test1@test.com";
            var agreement = new
            ...
            userRepositoryMock.Setup(x => x.GetEncryptedCreditCard(It.IsAny<string>()))
                .ReturnsAsync(null as CreditCard);
            var promotionRepositoryMock = new Mock<IPromotionRepository>();
            var factory = _factory.WithWebHostBuilder(builder =>
            {
                builder.ConfigureTestServices(services =>
